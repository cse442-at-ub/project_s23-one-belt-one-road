...
<body>
	...
	<main>
		<section id="sale_orders" style="text-align: center;">
			<h1 style="font-style: italic; margin-bottom: 30px;">Sale Orders</h1>
			<div class="orders-container">
				<?php
					require_once 'database_APIs/apiFunctions.php';
					$orders_result = getTransactionBySellerID($user_id);
					if ($orders_result == -1) {
					...
					if ($orders_result->num_rows == 0) {
				        echo '<p style="margin-block: 100px;">No past orders found. Post something to sell!</p>';
				    } else {
						while ($order_row = $orders_result->fetch_assoc()) {
							...
							echo '<div class="order-block-container">';
								...
								$order_number = hash('crc32', strval($order_row['sellerID']) . $order_row['datetime'] . $order_row['productID']);
							    ...
							    $product_result = getProductByID($order_row['productID']);
							    if ($product_result == -1) {
								...
								echo '<div class="item-blocks-long">';
									echo '<div id="order-item-block" class="item-block-long">';
										...
										echo '</div>';
										echo '<p class="order-item-quantity">Quantity: ' . $order_row['quantity'] . '</p>';
									echo '</div>';
								echo '</div>';
								
								echo '<div class="order-bottom-container">';
									echo '<p id="order-bottom-shipping-address" class="order-bottom-info">' . '</p>';
